...
function extractYouTubeTranscript() {
  // Selector for YouTube transcript segments
  ...
  segments.forEach(segment => {
    ...
    if (textDiv) {
      transcriptText.push(textDiv.textContent.trim());
    }
  });
...
function extractMetadata() {
  ...
  if (isYouTube) {
    // Try to get more context from YouTube
    const explicitDesc = document.querySelector('#description-inline-expander .ytd-text-inline-expander');
    if (explicitDesc) {
      description = explicitDesc.textContent.trim();
    }
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'extractText') {
    ...
    const text = extractYouTubeTranscript();
    sendResponse({ text: text, success: !!text });
  }
