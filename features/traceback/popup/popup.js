/**
 * Popup UI Controller for TraceBack
 */
...
async function checkAIAvailability() {
  try {
    ...
    // Check audio availability specifically
    startTranscription.audioAvailability = 'unknown'; // Default
    try {
      const audioAvail = await window.LanguageModel.availability({
        expectedInputs: [{ type: 'audio' }],
        expectedOutputs: [{ type: 'text', languages: ['en'] }]
      });
      console.log('🎤 Audio model availability:', audioAvail);
      startTranscription.audioAvailability = audioAvail;
    } catch (e) {
      console.warn('Could not check audio availability:', e);
    }
    ...
    if (availability === 'downloadable' || availability === 'downloading') {
      ...
      if (availability === 'downloading') {
        ...
        splashProgress.style.width = '50%'; // Show some progress to indicate activity
        splashProgressText.textContent = 'In Progress';

        // Check again after a delay to see if it finished
        setTimeout(() => checkAIAvailability(), 2000);
      } else {
    ...
    // Even if check fails, allow access after a delay if it's just a check error
    // but ideally show error. For now, let's just log it.
    splashCheckAPI.innerHTML = '<span class="error-icon">✕</span>';
...
async function initializeMainApp() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    // Transcription actions (delegated to fix CSP validation errors)
    const transcriptionBtn = e.target.closest('.js-transcription-btn');
    if (transcriptionBtn) {
      const action = transcriptionBtn.dataset.action;
      if (action === 'hideTranscription') {
        hideTranscriptionPage();
...
async function initializeAI() {
  try {
    ...
    if (availability === 'available') {
      console.log('✅ TraceBack: Prompt API ready! AI answers enabled.');
...
function showAISetupGuide(status, errorMsg = '') {
  ...
  console.log('\n' + '='.repeat(60));
  console.log('🤖 REMIND AI SETUP GUIDE');
...
/**
 * Extract captions from YouTube page via content script
 */
async function extractYouTubeCaptions(tabId) {
  try {
    ...
    if (response && response.success && response.text) {
      console.log('✅ Captions extracted:', response.text.substring(0, 50) + '...');

      // Setup fake transcription state to reuse existing UI
      isTranscribing = false;
      currentTranscript = response.text;
      transcriptionTabTitle = "YouTube Transcript"; // Or fetch actual title
      transcriptionStartTime = Date.now();

      // Show success UI
      showTranscriptionPage("YouTube Transcript");

      // Manually trigger completion handler logic (simplified)
      const transcriptionStatus = document.getElementById('transcriptionStatus');
      if (transcriptionStatus) transcriptionStatus.innerHTML = 'Complete';

      updateTranscriptDisplay(currentTranscript);

      // Show tab switcher and save button
      ['transcriptTabs', 'saveTranscriptHeaderBtn', 'transcriptText'].forEach(id => {
        const el = document.getElementById(id);
        if (el) {
          el.style.display = id === 'transcriptTabs' || id === 'saveTranscriptHeaderBtn' ? 'flex' : 'block';
      ...
      alert('Could not find open transcript. Please open the "Show Transcript" panel in the YouTube video description and try again.');
    }

  } catch (error) {
    console.error('Caption extraction failed:', error);
    alert('Failed to extract captions. Make sure the transcript panel is open.');
  }
}
...
async function startTranscription() {
  try {
    ...
    try {
      ...
      // DEBUG: Check specific audio availability status
      try {
        const audioStatus = await window.LanguageModel.availability({
          expectedInputs: [{ type: 'audio' }],
          expectedOutputs: [{ type: 'text', languages: ['en'] }]
        });
        console.log('🔍 Detailed Audio Model Status:', audioStatus);

        // Force download if status is 'after-download'
        if (audioStatus === 'after-download') {
          console.log('⬇️ Audio model needs download. Attempting to trigger...');
          try {
            const session = await window.LanguageModel.create({
              expectedInputs: [{ type: 'audio' }],
              expectedOutputs: [{ type: 'text', languages: ['en'] }],
            ...
            console.log('✅ Trigger session created!');
            session.destroy();
          } catch (e) {
            console.log('⬇️ Trigger creation failed (expected if downloading):', e);
          }
        }
      } catch (e) {
        console.error('🔍 Audio availability check error:', e);
      }

      // Create Prompt API session with audio input
      // First, try to create a session that accepts audio input
      try {
        ...
        console.error('Audio input session failed:', sessionError);


        const isYouTube = tab.url.includes('youtube.com/watch');
        // If it's YouTube, offer to extract captions
        if (isYouTube) {
          if (confirm('Audio model unavailable. Extract captions from YouTube transcript instead?')) {
            extractYouTubeCaptions(tab.id);
            return;
          }
        }

        let errorMessage = 'Your browser does not support audio transcription with the Gemini Nano model.';
        let detailedError = sessionError.message || sessionError.name;

        const status = startTranscription.audioAvailability;

        if (status === 'no') {
          errorMessage = 'Audio transcription is not supported by your browser/device.\n\nStatus: "no" (Hardware or permission issue).';
        } else if (status === 'downloadable' || status === 'after-download') {
          errorMessage = 'The audio transcription model needs to be downloaded.\n\nChrome should be downloading it now. Please wait a few minutes and try again.';
        } else if (sessionError.name === 'NotAllowedError') {
          errorMessage = 'Audio transcription capability is not available.\n\n1. Restart Chrome\n2. Check chrome://components > Optimization Guide On Device Model\n3. Ensure "Prompt API for Gemini Nano with Multimodal Input" is Enabled.';
        }

        alert(`${errorMessage}\n\nDebug: ${detailedError} (Avail: ${status})`);
        return;
      }
      ...
      try {
        ...
        let userMessage = 'Could not access tab audio.';
        if (error.name === 'NotAllowedError') {
          userMessage = 'Permission denied. Please allow audio sharing when prompted.\n\nNote: You must check "Share audio" when selecting the tab.';
        } else if (error.name === 'NotFoundError') {
          userMessage = 'No audio source found. Make sure the tab is playing audio.';
        } else if (error.name === 'AbortError') {
          userMessage = 'Audio capture was cancelled.';
        } else {
          userMessage = `Error accessing audio: ${error.message || error.name}`;
        }
      ...
      let errorMessage = recognitionError.message;
      if (recognitionError instanceof DOMException) {
        if (recognitionError.name === 'NotAllowedError') {
          errorMessage = 'Permission denied. Please allow audio sharing when prompted.\n\nNote: You must check "Share audio" when selecting the tab.';
        } else if (recognitionError.name === 'NotFoundError') {
          errorMessage = 'No audio source found. Make sure the tab is playing audio.';
        } else if (recognitionError.name === 'AbortError') {
          errorMessage = 'Audio capture was cancelled.';
        } else {
          errorMessage = `Audio access error: ${recognitionError.message || recognitionError.name}`;
        }
      } else if (recognitionError.name === 'NotAllowedError' || errorMessage.includes('capability')) {
        // Suppress alert for expected capability issues (text-only models)
        console.warn('TraceBack: Audio transcription not supported by this model. Falling back to text-only mode.');
        errorMessage = null;
      }
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === 'local') {
    ...
    if (hasNewScreenshot || hasNewTranscript) {
      console.log('TraceBack: New content detected, refreshing view...');
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'captureAdded') {
    console.log('TraceBack: New capture added notification received');
